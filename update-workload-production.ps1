# ═══════════════════════════════════════════════════════════
# 📚 ATUALIZAR CARGA HORÁRIA - PRODUÇÃO (RENDER)
# ═══════════════════════════════════════════════════════════
...
Write-Host "📚 ATUALIZAÇÃO DE CARGA HORÁRIA - PRODUÇÃO (RENDER)" -ForegroundColor White
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""

# ============================================================
# ⚠️ ATENÇÃO: ESTE SCRIPT ATUALIZA A PRODUÇÃO!
# ============================================================
Write-Host "⚠️  ATENÇÃO: Você está prestes a atualizar o sistema em PRODUÇÃO!" -ForegroundColor Red
Write-Host "   Certifique-se de que testou localmente primeiro." -ForegroundColor Yellow
Write-Host ""

$prodConfirm = Read-Host "   Deseja continuar com PRODUÇÃO? (Digite 'CONFIRMO' para prosseguir)"

if ($prodConfirm -ne "CONFIRMO") {
    Write-Host ""
...
# [COPIE AQUI TODOS OS DADOS DO $workloadData DO SCRIPT LOCAL]
# Os dados são os mesmos, apenas a URL da API muda
...
try {
    # 1️⃣ Login
    Write-Host "1️⃣  Autenticando no Render..." -ForegroundColor Yellow
    ...
    $backupFile = "backup-prod-workload-$timestamp.json"
    ...
    Write-Host "⚠️  CONFIRMAÇÃO FINAL - PRODUÇÃO" -ForegroundColor Red
    Write-Host "   Atualizar $($updates.Count) turma(s) no sistema de PRODUÇÃO?" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "7️⃣  Atualizando turmas em PRODUÇÃO..." -ForegroundColor Yellow
    Write-Host ""
    ...
    foreach ($update in $updates) {
        try {
            ...
            Write-Host "   ✅ [$percent%] $($update.className)" -ForegroundColor Green

            Start-Sleep -Milliseconds 500  # Delay maior para produção
    ...
    Write-Host ""
    Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
    Write-Host "📊 RESULTADO FINAL - PRODUÇÃO" -ForegroundColor White
    Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "🔗 Verificar: https://criador-horario-aula.surge.sh/class-subjects" -ForegroundColor Cyan
    Write-Host ""
    
    if ($errorCount -eq 0) {
        Write-Host "🎉 ATUALIZAÇÃO CONCLUÍDA COM SUCESSO EM PRODUÇÃO!" -ForegroundColor Green
    } else {
    ...
    if ($_.Exception.Response) {
        ...
        if ($statusCode -eq 429) {
            Write-Host ""
            Write-Host "⚠️  Rate limit ativo. Aguarde alguns minutos e tente novamente." -ForegroundColor Yellow
        }
    }
